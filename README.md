...
```sql
SELECT 
SUBSTRING(email, POSITION('@' IN email) + 1) AS domain_names 
FROM users 
```
...
```
```sql
INSERT INTO transactions (product_id, userid, spend, transaction_date)
VALUES
...
```
```sql
INSERT INTO office (emp_id, emp_status, time_id)
VALUES
